...
final class CodeActionTests: SourceKitLSPTestCase {
  ...
  private func validateCodeAction(
    ...
    guard let action = result?.codeActions?.first(where: { $0.title == title }) else {
      let available = result?.codeActions?.map { $0.title } ?? []
  ...
  func testConvertGuardToIfNotShownWithoutFollowingCode() async throws {
    ...
    let convertAction = result?.codeActions?.first { action in
