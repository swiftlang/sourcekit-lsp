...
final class ConvertDocumentationTests: XCTestCase {
  ...
  // MARK: Markdown and Tutorials

  func testMarkdownModule() async throws {
    ...
    try await convertDocumentation(
      fileName: "Module.md",
      project: project,
      expectedResponses: [.renderNode(.init(kind: .symbol, path: "MyLibrary"))]
    )
  }
  ...
  func testMarkdownExtension() async throws {
    let project = try await SwiftPMTestProject(
      files: [
        ...
        0️⃣# ``MyLibrary/thisIsNotAValidSymbol``
        """,
      ],
      enableBackgroundIndexing: true
    )
    try await convertDocumentation(
      ...
      project: project,
      expectedResponses: [
        .renderNode(.init(kind: .symbol, path: "MyLibrary/foo()")),
        .renderNode(.init(kind: .symbol, path: "MyLibrary/bar()")),
      ]
    )
    try await convertDocumentation(
      fileName: "Module.md",
    ...
    try await convertDocumentation(
      fileName: "Foo.md",
      project: project,
      expectedResponses: [.renderNode(.init(kind: .symbol, path: "MyLibrary/foo()"))]
    )
    try await convertDocumentation(
      fileName: "SymbolNotFound.md",
      project: project,
      expectedResponses: [.error(.symbolNotFound("MyLibrary/thisIsNotAValidSymbol"))]
    )
  }
  ...
  func testTutorial() async throws {
    ...
    try await convertDocumentation(
      ...
      project: project,
      expectedResponses: [.renderNode(.init(kind: .tutorial))]
    )
  }
  ...
  func testTutorialOverview() async throws {
    let project = try await SwiftPMTestProject(
      files: [
        ...
        0️⃣@Tutorials(name: "MyTutorialOverview") {
    ...
    try await convertDocumentation(
      ...
      project: project,
      expectedResponses: [.renderNode(.init(kind: .overview))]
    )
  }
}
