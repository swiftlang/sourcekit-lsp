...
final class CrashRecoveryTests: XCTestCase {
  func testSourcekitdCrashRecovery() {
    ...
    let expectedMarkupContent = MarkupContent(kind: .markdown, value: """
      # foo()
      ```
      func foo()
      ```
      """)
    let expectedHoverResponse = HoverResponse(contents: .markupContent(expectedMarkupContent), range: nil)
    ...
    precondition(preCrashHoverResponse == expectedHoverResponse, "Sanity check failed. The Hover response was not what we expected, even before crashing sourcekitd")

    // Crash sourcekitd

    let sourcekitdServer = ws.testServer.server!.languageService(for: loc.docUri, .swift, in: ws.testServer.server!.workspace!) as! SwiftLanguageServer
    ...
    // Check that we get the same hover response from the in-memory state
