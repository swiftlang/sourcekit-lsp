...
fileprivate extension SourceKitD {
  private func send(
    _ requestUid: KeyPath<sourcekitd_api_requests, sourcekitd_api_uid_t> & Sendable,
    _ request: SKDRequestDictionary
  ) async throws -> SKDResponseDictionary {
    try await self.send(requestUid, request, timeout: defaultTimeoutDuration, documentUrl: nil, fileContents: nil)
  }
