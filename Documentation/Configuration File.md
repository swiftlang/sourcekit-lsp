...
- `index`: Options related to indexing.
  ...
  - `swiftModuleCachePath: string`: Path to the Swift module cache for background preparation. If `nil` (default), SourceKit-LSP uses a platform-appropriate global module cache (macOS: `~/Library/Caches/org.swift.sourcekit-lsp/ModuleCache`, Linux: `$XDG_CACHE_HOME/sourcekit-lsp/ModuleCache` or `~/.cache/sourcekit-lsp/ModuleCache`). Set to empty string to disable module cache sharing.
