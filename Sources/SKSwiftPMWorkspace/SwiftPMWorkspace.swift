...
public final class SwiftPMWorkspace {
      ...
      public init(
            workspacePath: AbsolutePath, toolchainRegistry: ToolchainRegistry,
            ...
            guard
                  let packageRoot = findPackageDirectory(
                  containing: workspacePath, fileSystem)
            else { throw Error.noManifest(workspacePath: workspacePath) }
            ...
            if case .darwin? = Platform.currentPlatform {
                  if let path = try? Process.checkNonZeroExit(
                        args: "/usr/bin/xcrun", "--show-sdk-path", "--sdk",
                        "macosx")
                  { sdk = try? AbsolutePath(validating: path.spm_chomp()) }
                  if let path = try? Process.checkNonZeroExit(
                        args: "/usr/bin/xcrun", "--show-sdk-platform-path",
                        "--sdk", "macosx")
                  {
                        platformPath = try? AbsolutePath(
                              validating: path.spm_chomp())
                  }
            }
            ...
            if let platformPath = platformPath {
                  let flags = [
                        "-F",
                        platformPath.appending(
                              components: "Developer", "Library", "Frameworks").asString,
                  ]
            ...
            self.workspace = Workspace(
                  ...
                  pinsFile: packageRoot.appending(
                        component: "Package.resolved"),
                  manifestLoader: ManifestLoader(
                        manifestResources: swiftPMToolchain),
                  delegate: BuildSettingProviderWorkspaceDelegate(),
                  fileSystem: fileSystem, skipUpdate: true)
            ...
            self.buildParameters = BuildParameters(
                  ...
                  configuration: .debug, toolchain: swiftPMToolchain,
                  flags: BuildFlags())
      ...
      public convenience init?(
            url: LanguageServerProtocol.URL,
            ...
            do {
                  ...
                  log(
                        "could not find manifest, or not a SwiftPM package: \(path.asString)",
                        level: .warning)
                  return nil
            }
            catch {
                  log(
                        "failed to create \(SwiftPMWorkspace.self): \(error)",
                        level: .error)
                  return nil
            }
      }
      ...
      func reloadPackage() throws {
            ...
            let diags = DiagnosticsEngine(
                  handlers: [
                        { diag in
                              log(
                                    diag.localizedDescription,
                                    level: diag.behavior.asLogLevel)
                        }
                  ])
            ...
            let plan = try BuildPlan(
                  buildParameters: buildParameters, graph: packageGraph,
                  diagnostics: diags, fileSystem: fileSystem)
...
extension SwiftPMWorkspace: BuildSystem {
      ...
      public var buildPath: AbsolutePath { return buildParameters.buildPath }

      public var indexStorePath: AbsolutePath? {
            return buildParameters.indexStoreMode == .off
                  ? nil : buildParameters.indexStore
      }
      ...
      public func settings(
            for url: LanguageServerProtocol.URL, _ language: Language
      ) -> FileBuildSettings? {
...
extension SwiftPMWorkspace {
      ...
      func settings(forPackageManifest path: AbsolutePath) -> FileBuildSettings?
      {
            ...
            where path == package.manifest.path {
                  let compilerArgs = workspace.interpreterFlags(
                        for: package.path) + [path.asString]
                  return FileBuildSettings(
                        preferredToolchain: nil, compilerArguments: compilerArgs
                  )
            }
            return nil
      }
      ...
      public func settings(forHeader path: AbsolutePath, _ language: Language)
            -> FileBuildSettings?
      {
      ...
      public func settings(
            forSwiftFile path: AbsolutePath, _ td: SwiftTargetBuildDescription
      ) -> FileBuildSettings? {
            // FIXME: this is re-implementing llbuild's constructCommandLineArgs.
            var args: [String] = [
                  "-module-name", td.target.c99name, "-incremental",
                  "-emit-dependencies", "-emit-module", "-emit-module-path",
                  buildPath.appending(
                        component: "\(td.target.c99name).swiftmodule").asString,// -output-file-map <path>
            ]
      ...
      public func settings(
            forClangFile path: AbsolutePath, _ language: Language,
            ...
            let compilePath
                  = td.compilePaths().first(where: { $0.source == path })
            ...
            if let compilePath = compilePath {
                  args += [
                        "-c", compilePath.source.asString, "-o",
                        compilePath.object.asString,
                  ]
            }
            else if path.extension == "h" {
                  args += ["-c"]
                  if let xflag = language.xflagHeader { args += ["-x", xflag] }
                  args += [path.asString]
            }
            else { args += ["-c", path.asString,] }
...
extension ToolchainRegistry {
      ...
      fileprivate var swiftPMHost: SwiftPMToolchain? {
            ...
            if swiftc == nil || clang == nil { return nil }

            return SwiftPMToolchain(
                  swiftCompiler: swiftc!, clangCompiler: clang!,
                  libDir: swiftc!.parentDirectory.parentDirectory.appending(
                        components: "lib", "swift", "pm"), sdkRoot: nil,
                  extraCCFlags: [], extraSwiftCFlags: [], extraCPPFlags: [],
                  dynamicLibraryExtension:
                        Platform.currentPlatform?.dynamicLibraryExtension
                        ?? "so")
      }
}
...
private func findPackageDirectory(
      containing path: AbsolutePath, _ fileSystem: FileSystem
) -> AbsolutePath? {
      var path = path
      while !fileSystem.isFile(path.appending(component: "Package.swift")) {
            if path.isRoot { return nil }
...
public final class BuildSettingProviderWorkspaceDelegate: WorkspaceDelegate {
      ...
      public func fetchingDidFinish(
            repository: String, diagnostic: Basic.Diagnostic?
      ) {}
      ...
      public func managedDependenciesDidUpdate(
            _ dependencies: AnySequence<ManagedDependency>
      ) {}
}
