...
public final class ToolchainRegistry {
      ...
      var queue: DispatchQueue = DispatchQueue(
            label: "toolchain-registry-queue")
...
extension ToolchainRegistry {
      ...
      public static var shared: ToolchainRegistry = ToolchainRegistry(
            localFileSystem)
...
extension ToolchainRegistry {
      ...
      public var `default`: Toolchain? {
            get {
                  return queue.sync {
                        if _default == nil {
                              if let tc = toolchainIdentifiers[
                                    darwinToolchainIdentifier]
                              { _default = tc }
                              else { _default = _toolchains.first }
                        }
                        return _default
                  }
            }
            ...
            set {
                  queue.sync {
                        ...
                        precondition(
                              toolchainIdentifiers[toolchain.identifier]
                                    === toolchain,
      ...
      public static let darwinDefaultToolchainIdentifier: String
      ...
      public var darwinToolchainIdentifier: String {
            return darwinToolchainOverride
                  ?? ToolchainRegistry.darwinDefaultToolchainIdentifier
      }

      /// All toolchains, in the order they were added.
      public var toolchains: [Toolchain] { return queue.sync { _toolchains } }
...
extension ToolchainRegistry {
      ...
      public func registerToolchain(
            _ path: AbsolutePath, _ fileSystem: FileSystem = localFileSystem
      ) throws -> Toolchain {
            return try queue.sync { try _registerToolchain(path, fileSystem) }
      }

      func _registerToolchain(_ path: AbsolutePath, _ fileSystem: FileSystem)
            throws -> Toolchain
      {
            ...
            else { throw Error.invalidToolchain }
      }
}
...
extension ToolchainRegistry {
      ...
      public func scanForToolchains(
            ...
            xcodes: [AbsolutePath] = [currentXcodeDeveloperPath
            ].compactMap({ $0 }),
            xctoolchainSearchPaths: [AbsolutePath] = [
                  AbsolutePath(
                        expandingTilde: "~/Library/Developer/Toolchains"),
            ...
            queue.sync {
                  _scanForToolchains(
                        ...
                        setDefault: true, fileSystem)
                  xcodes.forEach { _scanForToolchains(xcode: $0, fileSystem) }
                  xctoolchainSearchPaths.forEach {
                        _scanForToolchains(
                              xctoolchainSearchPath: $0, fileSystem)
                  }
      ...
      public func scanForToolchains(
            environmentVariables: [String], setDefault: Bool,
            _ fileSystem: FileSystem = localFileSystem
      ) {
            queue.sync {
                  _scanForToolchains(
                        environmentVariables: environmentVariables,
                        setDefault: setDefault, fileSystem)
            }
      }
      ...
      func _scanForToolchains(
            ...
            for envVar in environmentVariables {
                  if let pathStr = getenv(envVar),
                        ...
                        let toolchain = try? _registerToolchain(
                        path, fileSystem), shouldSetDefault
                  {
      ...
      ///   - setDefault: If true, the first toolchain found will be set as the default.
      public func scanForToolchains(
            pathVariables: [String], _ fileSystem: FileSystem = localFileSystem
      ) {
            queue.sync {
                  _scanForToolchains(pathVariables: pathVariables, fileSystem)
            }
      ...
      func _scanForToolchains(pathVariables: [String], _ fileSystem: FileSystem)
      {
            pathVariables.lazy.flatMap { envVar in
                  getEnvSearchPaths(
                        pathString: getenv(envVar), currentWorkingDirectory: nil
                  )
            }.forEach { path in
      ...
      public func scanForToolchains(
            xcode: AbsolutePath, _ fileSystem: FileSystem = localFileSystem
      ...
      func _scanForToolchains(xcode: AbsolutePath, _ fileSystem: FileSystem) {
            ...
            _scanForToolchains(
                  xctoolchainSearchPath: path.appending(
                        component: "Toolchains"), fileSystem)
      }
      ...
      public func scanForToolchains(
            ...
            queue.sync {
                  _scanForToolchains(
                        xctoolchainSearchPath: searchPath, fileSystem)
            }
      }
      ...
      func _scanForToolchains(
            ...
            guard
                  let direntries = try? fileSystem.getDirectoryContents(
                  searchPath)
            else { return }
      ...
      public static var currentXcodeDeveloperPath: AbsolutePath? {
            if let str = try? Process.checkNonZeroExit(
                  args: "/usr/bin/xcode-select", "-p"),
