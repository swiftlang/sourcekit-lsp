...
public final class Toolchain {
      ...
      public init(
            identifier: String, displayName: String, path: AbsolutePath? = nil,
            clang: AbsolutePath? = nil, swiftc: AbsolutePath? = nil,
            clangd: AbsolutePath? = nil, sourcekitd: AbsolutePath? = nil,
      ...
      convenience public init?(
                  ...
                  "", { try XCToolchainPlist(fromDirectory: path, fileSystem) })
            {
                  self.init(
                        ...
                        displayName: infoPlist.displayName ?? String(
                              path.basename.dropLast(path.suffix?.count ?? 0)),
                        path: path)
            }
            else {
                  self.init(
                        identifier: path.asString, displayName: path.basename,
                        path: path)
            }

            if !searchForTools(path, fileSystem) { return nil }
      }

      /// Search `path` for tools, returning true if any are found.
      @discardableResult func searchForTools(
            _ path: AbsolutePath, _ fs: FileSystem = localFileSystem
      ) -> Bool {
            return searchForTools(binPath: path, fs) || searchForTools(
                  binPath: path.appending(components: "bin"), fs)
                  || searchForTools(
                  binPath: path.appending(components: "usr", "bin"), fs)
      }

      private func searchForTools(binPath: AbsolutePath, _ fs: FileSystem)
            -> Bool
      {

            let libPath = binPath.parentDirectory.appending(component: "lib")

            guard fs.isDirectory(binPath) || fs.isDirectory(libPath) else {
                  return false
            }
            ...
            let dylibExt = Platform.currentPlatform?.dynamicLibraryExtension
                  ?? "so"

            let sourcekitdPath = libPath.appending(
                  components: "sourcekitd.framework", "sourcekitd")
            if fs.isFile(sourcekitdPath) {
                  ...
                  let sourcekitdPath = libPath.appending(
                        component: "libsourcekitdInProc.\(dylibExt)")
                  if fs.isFile(sourcekitdPath) {
                        self.sourcekitd = sourcekitdPath
                        foundAny = true
                  }
            ...
            let libIndexStore = libPath.appending(
                  components: "libIndexStore.\(dylibExt)")
