...
extension XCToolchainPlist {
      ...
      enum Error: Swift.Error { case unsupportedPlatform }
      ...
      init(
            fromDirectory path: AbsolutePath,
      ...
      ///
      /// - parameter path: The directory to search.
      init(path: AbsolutePath, _ fileSystem: FileSystem = localFileSystem)
            throws
      {
            #if os(macOS)
                  ...
                  self = try bytes.withUnsafeData { data in
                        ...
                        return try decoder.decode(
                              XCToolchainPlist.self, from: data, format: &format
                        )
                  }
            #else
                  throw Error.unsupportedPlatform
...
extension XCToolchainPlist: Codable {
      ...
      init(from decoder: Decoder) throws {
            ...
            if let identifier = try container.decodeIfPresent(
                  String.self, forKey: .Identifier)
            { self.identifier = identifier }
            else {
                  self.identifier = try container.decode(
                        String.self, forKey: .CFBundleIdentifier)
            }
