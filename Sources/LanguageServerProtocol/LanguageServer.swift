...
open class LanguageServerEndpoint {
      ...
      open func _logResponse<Response>(
            _ result: LSPResult<Response>, id: RequestID, method: String
      ) {
            logAsync { _ in
                  """
...
extension LanguageServerEndpoint {
      ...
      public func _register<Server, R>(
            _ requestHandler: @escaping (Server) -> (Request<R>) -> Void
      ) {
            // We can use `unowned` here because the handler is run synchronously on `queue`.
            precondition(self is Server)
            requestHandlers[ObjectIdentifier(R.self)] = {
      ...
      ///
      /// Must be called on `queue`.
      public func _register<Server, N>(
            _ noteHandler: @escaping (Server) -> (Notification<N>) -> Void
      ) {
            // We can use `unowned` here because the handler is run synchronously on `queue`.
            notificationHandlers[ObjectIdentifier(N.self)] = {
      ...
      ///
      /// Must be called on `queue`.
      public func _register<N>(
            _ noteHandler: @escaping (Notification<N>) -> Void
...
extension LanguageServerEndpoint: MessageHandler {
      ...
      public func handle<N>(_ params: N, from clientID: ObjectIdentifier)
      where N: NotificationType {
            queue.async {
                  ...
                  guard
                        let handler = self.notificationHandlers[
                        ObjectIdentifier(N.self)]
                        as? ((Notification<N>) -> Void) else {
      ...
      public func handle<R>(
            ...
            queue.async {
                  ...
                  let request = Request(
                        params, id: id, clientID: clientID,
                        cancellation: cancellationToken,
                        reply: { [weak self] result in
                              self?.requestCancellation[key] = nil
                              reply(result)
                              self?._logResponse(
                                    result, id: id, method: R.method)
                        })

                  self._logRequest(request)

                  guard
                        let handler = self.requestHandlers[
                        ObjectIdentifier(R.self)] as? ((Request<R>) -> Void)
                  else {
