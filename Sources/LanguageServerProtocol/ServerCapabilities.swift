...
public struct ServerCapabilities: Codable, Hashable {
      ...
      public var documentOnTypeFormattingProvider:
      ...
      public init(
            ...
            hoverProvider: Bool? = nil, definitionProvider: Bool? = nil,
            ...
            documentOnTypeFormattingProvider: DocumentOnTypeFormattingOptions?
                  = nil, foldingRangeProvider: Bool? = nil
      ) {
      ...
      public init(from decoder: Decoder) throws {
            ...
            self.completionProvider = try container.decodeIfPresent(
                  CompletionOptions.self, forKey: .completionProvider)
            self.hoverProvider = try container.decodeIfPresent(
                  Bool.self, forKey: .hoverProvider)
            self.definitionProvider = try container.decodeIfPresent(
                  Bool.self, forKey: .definitionProvider)
            self.foldingRangeProvider = try container.decodeIfPresent(
                  Bool.self, forKey: .foldingRangeProvider)

            if let textDocumentSync = try? container.decode(
                  TextDocumentSyncOptions.self, forKey: .textDocumentSync)
            {
                  self.textDocumentSync = textDocumentSync

            }
            else if let kind = try? container.decode(
                  TextDocumentSyncKind.self, forKey: .textDocumentSync)
            {
                  // Legacy response
                  self.textDocumentSync = TextDocumentSyncOptions(
                        openClose: nil, change: kind, willSave: nil,
                        willSaveWaitUntil: nil, save: nil)

            }
            else { self.textDocumentSync = nil }
      }
}
...
public struct TextDocumentSyncOptions: Codable, Hashable {
      ...
      public init(
            ...
            willSave: Bool? = true, willSaveWaitUntil: Bool? = false,
