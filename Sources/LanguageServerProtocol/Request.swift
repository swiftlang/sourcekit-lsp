...
public final class Request<R: RequestType> {
      ...
      private var replied: Bool = false {
            willSet {
                  precondition(
                        replied == false, "replied to \(id) more than once")
            }
      }
      ...
      public init(
            _ request: Params, id: RequestID, clientID: ObjectIdentifier,
      ...
      deinit { precondition(replied, "request \(id) never received a reply") }
      ...
      public func reply(_ result: Response) { reply(.success(result)) }
