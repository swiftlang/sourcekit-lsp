...
struct CursorInfo {
      ...
      init(
            _ symbolInfo: SymbolDetails, annotatedDeclaration: String?,
...
extension CursorInfo {
      ...
      init?(
            _ dict: SKResponseDictionary, _ snapshot: DocumentSnapshot,
            ...
            if let filepath: String = dict[keys.filepath],
                  ...
                  location = Location(
                        url: URL(fileURLWithPath: filepath), range: Range(pos))
            }

            self.init(
                  SymbolDetails(
                        name: dict[keys.name], containerName: nil,
                        usr: dict[keys.usr], bestLocalDeclaration: location),
...
extension SwiftLanguageServer {
      ...
      func cursorInfo(
            _ url: URL, _ position: Position,
            _ completion: @escaping (Result<CursorInfo?, CursorInfoError>) ->
                  Void
      ) {
            ...
            let handle = sourcekitd.send(skreq) { [weak self] result in
                  ...
                  guard let dict = result.success else {
                        return completion(
                              .failure(.responseError(result.failure!)))
                  }
                  ...
                  completion(
                        .success(
                              CursorInfo(
                                    ...
                                    annotatedDeclaration: dict[
                                          self.keys.annotated_decl],
                                    documentationXML: dict[
                                          self.keys.doc_full_as_xml])))
            }
