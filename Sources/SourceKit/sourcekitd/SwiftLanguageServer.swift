...
public final class SwiftLanguageServer: LanguageServer {
      ...
      public init(
            client: Connection, sourcekitd: AbsolutePath,
            buildSystem: BuildSystem, clientCapabilities: ClientCapabilities,
      ...
      func getDiagnostic(
            _ diag: SKResponseDictionary, for snapshot: DocumentSnapshot
      ) -> Diagnostic? {

            // FIXME: this assumes that the diagnostics are all in the same file.

            guard let message: String = diag[keys.description] else {
                  return nil
            }

            var position: Position? = nil
            if let line: Int = diag[keys.line],
                  let utf8Column: Int = diag[keys.column], line > 0, utf8Column
                  > 0
            {
                  position = snapshot.positionOf(
                        zeroBasedLine: line - 1, utf8Column: utf8Column - 1)
            }
            ...
            if let uid: sourcekitd_uid_t = diag[keys.severity] {
                  switch uid {
                  case values.diag_error: severity = .error
                  case values.diag_warning: severity = .warning
                  default: break
                  }
            }
            ...
            if let sknotes: SKResponseArray = diag[keys.diagnostics] {
                  ...
                  sknotes.forEach { (_, sknote) -> Bool in
                        guard let note = getDiagnostic(sknote, for: snapshot)
                        else { return true }
                        notes?.append(
                              DiagnosticRelatedInformation(
                                    location: Location(
                                          url: snapshot.document.url,
                                          range: note.range.asRange),
            ...
            return Diagnostic(
                  range: Range(position!), severity: severity, code: nil,
                  source: "sourcekitd", message: message,
                  relatedInformation: notes)
      }

      func publicDiagnostics(
            _ diags: SKResponseArray?, for snapshot: DocumentSnapshot
      ) {
            ...
            client.send(
                  PublishDiagnostics(
                        url: snapshot.document.url, diagnostics: result))
      }
...
extension SwiftLanguageServer {
      ...
      func initialize(_ request: Request<InitializeRequest>) {
            ...
            api.set_notification_handler { [weak self] notification in
                  ...
                  let notification = SKResponse(
                        notification, sourcekitd: self.sourcekitd)
                  ...
                  InitializeResult(
                        capabilities: ServerCapabilities(
                              textDocumentSync: TextDocumentSyncOptions(
                                    ...
                                    save: TextDocumentSyncOptions.SaveOptions(
                                          includeText: false)),
                              completionProvider: CompletionOptions(
                                    resolveProvider: false,
                                    triggerCharacters: ["."]),
                              hoverProvider: true, definitionProvider: nil,
      ...
      func cancelRequest(_ notification: Notification<CancelRequest>) {
            let key = RequestCancelKey(
                  client: notification.clientID, request: notification.params.id
            )
      ...
      func openDocument(_ note: Notification<DidOpenTextDocument>) {
            ...
            if let settings = buildSystem.settings(
                  for: snapshot.document.url, snapshot.document.language)
      ...
      func changeDocument(_ note: Notification<DidChangeTextDocument>) {
            ...
            let snapshot = documentManager.edit(note) {
                  (
                  ...
                  if let range = edit.range {
                        ...
                        guard
                              let offset = before.utf8Offset(
                              of: range.lowerBound),
                              let end = before.utf8Offset(of: range.upperBound)
                        else { fatalError("invalid edit \(range)") }
      ...
      func completion(_ req: Request<CompletionRequest>) {
            ...
            guard
                  let snapshot = documentManager.latestSnapshot(
                  req.params.textDocument.url)
            else {
                  log(
                        "failed to find snapshot for url \(req.params.textDocument.url)"
                  )
            ...
            guard
                  let completionPos = adjustCompletionLocation(
                  req.params.position, in: snapshot)
            else {
            ...
            guard let offset = snapshot.utf8Offset(of: completionPos) else {
                  log(
                        "invalid completion position \(req.params.position) (adjusted: \(completionPos)"
                  )
                  req.reply(CompletionList(isIncomplete: true, items: []))
                  return
            }
            ...
            let handle = sourcekitd.send(skreq) { [weak self] result in
                  ...
                  guard
                        let completions: SKResponseArray = dict[
                        self.keys.results]
                  else {
                        req.reply(
                  ...
                  let cancelled = !completions.forEach { (i, value) -> Bool in
                        guard let name: String = value[self.keys.description]
                        else {
                        ...
                        result.items.append(
                              CompletionItem(
                                    label: name, detail: nil, sortText: nil,
                                    filterText: filterName, textEdit: nil,
                                    insertText: insertText.map {
                                          self.rewriteCompletionPlacholders($0)
                                    }, insertTextFormat: .snippet,
                                    kind: kind?.asCompletionItemKind(
                                          self.values) ?? .value,
                                    deprecated: nil))
      ...
      func rewriteCompletionPlacholders(_ completion: String) -> String {
            if !completion.contains("<#") { return completion }

            var result = completion
            var index = 1
            while let start = result.range(of: "<#") {
                  guard let end = result[start.upperBound...].range(of: "#>")
                  else {
                        log(
                              "invalid placholder in \(completion)",
                              level: .debug)
                        return completion
                  }
      ...
      func adjustCompletionLocation(
            _ pos: Position, in snapshot: DocumentSnapshot
      ) -> Position? {
            ...
            let identifierChars = CharacterSet.alphanumerics.union(
                  CharacterSet(charactersIn: "_"))

            guard
                  var loc = lineSlice.utf16.index(
                  startIndex, offsetBy: pos.utf16index,
            ...
            let adjustedOffset = lineSlice.utf16.distance(
                  from: startIndex, to: loc)
      ...
      func hover(_ req: Request<HoverRequest>) {
            ...
            cursorInfo(url, position) { result in
                  guard let cursorInfo: CursorInfo = result.success ?? nil
                  else {
                        if let error = result.failure {
                              log(
                                    "cursor info failed \(url):\(position): \(error)",
                                    level: .warning)
                        }
        ...
        """
                  ...
                  else if let annotated: String
                        = cursorInfo.annotatedDeclaration
                  {
                        result
                              += """
                  ...
                  req.reply(
                        HoverResponse(
                              contents: MarkupContent(
                                    kind: .markdown, value: result), range: nil)
                  )
            }
      }
      ...
      func documentSymbolHighlight(_ req: Request<DocumentHighlightRequest>) {
            ...
            guard let offset = snapshot.utf8Offset(of: req.params.position)
            else {
            ...
            let handle = sourcekitd.send(skreq) { [weak self] result in
                  ...
                  guard let results: SKResponseArray = dict[self.keys.results]
                  ...
                  results.forEach { _, value in
                        if let offset: Int = value[self.keys.offset],
                              let start: Position = snapshot.positionOf(
                              utf8Offset: offset),
                              let length: Int = value[self.keys.length],
                              let end: Position = snapshot.positionOf(
                              utf8Offset: offset + length)
                        {
                              highlights.append(
                                    DocumentHighlight(
                                          range: start..<end,
      ...
      func foldingRange(_ req: Request<FoldingRangeRequest>) {
            ...
            let handle = sourcekitd.send(skreq) { [weak self] result in
                  ...
                  guard
                        let syntaxMap: SKResponseArray = dict[
                        self.keys.syntaxmap],
                        let substructure: SKResponseArray = dict[
                        self.keys.substructure]
                  else { return req.reply([]) }
                  ...
                  var hasReachedLimit: Bool {
                        let capabilities
                              = self.clientCapabilities.textDocument?.foldingRange
                  ...
                  syntaxMap.forEach { _, value in
                        if let kind: sourcekitd_uid_t = value[self.keys.kind],
                              ...
                              let length: Int = value[self.keys.length] {
                              if let comment = currentComment, comment.offset
                                    + comment.length == offset
                              {
                                    currentComment!.length += length
                                    return true
                              }
                              if let comment = currentComment {
                                    self.addFoldingRange(
                                          offset: comment.offset,
                                          length: comment.length,
                                          kind: .comment, in: snapshot,
                                          toArray: &ranges)
                              }
                  ...
                  if let comment = currentComment, hasReachedLimit == false {
                        self.addFoldingRange(
                              offset: comment.offset, length: comment.length,
                              kind: .comment, in: snapshot, toArray: &ranges)
                        currentComment = nil
                  }

                  var structureStack: [SKResponseArray] = [substructure]
                  while !hasReachedLimit,
                        let substructure = structureStack.popLast()
                  {
                        substructure.forEach { _, value in
                              if let offset: Int = value[self.keys.bodyoffset],
                                    let length: Int = value[
                                    self.keys.bodylength], length > 0
                              {
                                    self.addFoldingRange(
                                          offset: offset, length: length,
                                          in: snapshot, toArray: &ranges)
                                    if hasReachedLimit { return false }
                              }
                              if let substructure: SKResponseArray = value[
                                    self.keys.substructure]
      ...
      func addFoldingRange(
            offset: Int, length: Int, kind: FoldingRangeKind? = nil,
            in snapshot: DocumentSnapshot, toArray ranges: inout [FoldingRange]
      ) {
            guard let start: Position = snapshot.positionOf(utf8Offset: offset),
                  ...
                  log(
                        "folding range failed to retrieve position of \(snapshot.document.url): \(offset)-\(offset + length)",
                        level: .warning)
                  return
            }
            ...
            if capabilities?.lineFoldingOnly == true {
                  ...
                  if lastLineToFold <= start.line { return }
                  else {
                        range = FoldingRange(
                              startLine: start.line, startUTF16Index: nil,
                              endLine: lastLineToFold, endUTF16Index: nil,
                              kind: kind)
                  }
            }
            else {
                  range = FoldingRange(
                        ...
                        startUTF16Index: start.utf16index, endLine: end.line,
                        endUTF16Index: end.utf16index, kind: kind)
            }
            ranges.append(range)
      }
}
...
extension DocumentSnapshot {
      ...
      func utf8Offset(of pos: Position) -> Int? {
            return lineTable.utf8OffsetOf(
                  line: pos.line, utf16Column: pos.utf16index)
      }
      ...
      func positionOf(zeroBasedLine: Int, utf8Column: Int) -> Position? {
            return lineTable.utf16ColumnAt(
                  line: zeroBasedLine, utf8Column: utf8Column).map
...
func makeLocalSwiftServer(
      ...
      buildSettings: BuildSystem?, clientCapabilities: ClientCapabilities?
) throws -> Connection {
...
extension sourcekitd_uid_t {
      func isCommentKind(_ vals: sourcekitd_values) -> Bool {
            switch self {
            case vals.syntaxtype_comment, vals.syntaxtype_comment_marker,
                  vals.syntaxtype_comment_url:
                  return true
            default: return isDocCommentKind(vals)
            }
      }

      func isDocCommentKind(_ vals: sourcekitd_values) -> Bool {
            return self == vals.syntaxtype_doccomment || self
                  == vals.syntaxtype_doccomment_field
      }

      func asCompletionItemKind(_ vals: sourcekitd_values)
            -> CompletionItemKind?
      {
            switch self {
            case vals.kind_keyword: return .keyword
            case vals.decl_module: return .module
            case vals.decl_class: return .class
            case vals.decl_struct: return .struct
            case vals.decl_enum: return .enum
            case vals.decl_enumelement: return .enumMember
            case vals.decl_protocol: return .interface
            case vals.decl_associatedtype: return .typeParameter
            case vals.decl_typealias:
                  return .typeParameter  // FIXME: is there a better choice?
            case vals.decl_generic_type_param: return .typeParameter
            case vals.decl_function_constructor: return .constructor
            ...
            case vals.decl_function_method_static: return .method
            case vals.decl_function_method_instance: return .method
            case vals.decl_function_operator_prefix,
                  vals.decl_function_operator_postfix,
                  vals.decl_function_operator_infix:
                  return .operator
            case vals.decl_precedencegroup: return .value
            case vals.decl_function_free: return .function
            case vals.decl_var_static, vals.decl_var_class: return .property
            case vals.decl_var_instance: return .property
            case vals.decl_var_local, vals.decl_var_global,
