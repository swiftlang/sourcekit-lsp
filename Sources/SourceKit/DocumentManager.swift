...
public final class Document {
      ...
      fileprivate var latestSnapshot: DocumentSnapshot {
            return DocumentSnapshot(
                  document: self, version: latestVersion,
...
public final class DocumentManager {
      ...
      @discardableResult public func open(
            _ url: URL, language: Language, version: Int, text: String
      ...
      @discardableResult public func edit(
            _ url: URL, newVersion: Int,
            edits: [TextDocumentContentChangeEvent],
            editCallback:
                  (
                  (_ before: DocumentSnapshot, TextDocumentContentChangeEvent)
                        -> Void
            )? = nil
      ) throws -> DocumentSnapshot {
            return try queue.sync {
      ...
      public func latestSnapshot(_ url: URL) -> DocumentSnapshot? {
            return queue.sync {
                  guard let document = documents[url] else { return nil }
                  return document.latestSnapshot
            }
      }
}
...
extension DocumentManager {
      ...
      @discardableResult func open(_ note: Notification<DidOpenTextDocument>)
            -> DocumentSnapshot?
      {
            let doc = note.params.textDocument
            return orLog("failed to open document", level: .error) {
                  try open(
                        doc.url, language: doc.language, version: doc.version,
                        text: doc.text)
            }
      }
      ...
      @discardableResult func edit(
            _ note: Notification<DidChangeTextDocument>,
            editCallback:
                  (
                  (_ before: DocumentSnapshot, TextDocumentContentChangeEvent)
                        -> Void
            ...
            return orLog("failed to edit document", level: .error) {
                  try edit(
                        ...
                        newVersion: note.params.textDocument.version ?? -1,
                        edits: note.params.contentChanges,
