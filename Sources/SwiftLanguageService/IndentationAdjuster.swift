...
/// SyntaxRewriter that adjusts indentation for lines starting with a newline.
class IndentationAdjuster: SyntaxRewriter {
  enum Mode {
    case add(Trivia)
    case remove(Trivia)
  }
  ...
  override func visit(_ token: TokenSyntax) -> TokenSyntax {
    ...
    while i < pieces.count {
      if pieces[i].isNewline {
        ...
        switch mode {
        case .add(let indent):
          pieces.insert(contentsOf: indent.pieces, at: i)
          i += indent.pieces.count
        case .remove(let indent):
          let pattern = indent.pieces
    ...
    for j in 0..<pieces.count {
      if case .blockComment(let text) = pieces[j] {
        pieces[j] = .blockComment(adjustInternalIndentation(of: text))
      }
    }
  ...
  private func adjustInternalIndentation(of text: String) -> String {
    ...
    for line in lines.dropFirst() {
      switch mode {
      case .add(let indent):
        adjustedLines.append(indent.description + line)
      case .remove(let indent):
        var adjustedLine = line
        let pattern = indent.description
