...
public final class JSONRPCConection {
      ...
      let queue: DispatchQueue = DispatchQueue(
            label: "jsonrpc-queue", qos: .userInitiated)
      let sendQueue: DispatchQueue = DispatchQueue(
            label: "jsonrpc-send-queue", qos: .userInitiated)
      let receiveIO: DispatchIO
      let sendIO: DispatchIO

      enum State { case created, running, closed }
      ...
      public init(
            inFD: Int32, outFD: Int32, closeHandler: @escaping () -> Void = {}
      ) {
            state = .created
            self.closeHandler = closeHandler

            receiveIO = DispatchIO(
                  type: .stream, fileDescriptor: inFD, queue: queue
            ) { (error: Int32) in
                  if error != 0 { log("IO error \(error)", level: .error) }
            }

            sendIO = DispatchIO(
                  type: .stream, fileDescriptor: outFD, queue: sendQueue
            ) { (error: Int32) in
                  if error != 0 { log("IO error \(error)", level: .error) }
            }
      ...
      public func start(receiveHandler: MessageHandler) {
            ...
            receiveIO.read(offset: 0, length: Int.max, queue: queue) {
                  ...
                  if self.requestBuffer.isEmpty {
                        data.withUnsafeBytes {
                              ...
                              let rest = self.parseAndHandleMessages(
                                    from: UnsafeBufferPointer(
                                          start: pointer, count: data.count))
                        ...
                        self.requestBuffer.removeFirst(
                              self.requestBuffer.count - unused)
                  }
            }
      }
      ...
      func readyToSend(shouldLog: Bool = true) -> Bool {
            precondition(
                  state != .created,
                  "tried to send message before calling start(messageHandler:)")
            let ready = state == .running
            if shouldLog && !ready { log("ignoring message; state = \(state)") }
            return ready
      }

      /// Parse and handle all messages in `bytes`, returning a slice containing any remaining incomplete data.
      func parseAndHandleMessages(from bytes: UnsafeBufferPointer<UInt8>)
            -> UnsafeBufferPointer<UInt8>.SubSequence {
                  ...
                  do {
                        ...
                        let pointer = UnsafeMutableRawPointer(
                              mutating: UnsafeBufferPointer(
                                    rebasing: messageBytes).baseAddress!)
                        ...
                        case .response:
                              if let id = error.id {
                                    if let outstanding
                                          = self.outstandingRequests.removeValue(
                                          forKey: id)
                                    {
                                          outstanding.replyHandler(
                                                .failure(ResponseError(error)))
                                    }
                                    else {
                                          log(
                                                "error in response to unknown request \(id) \(error)",
                                                level: .error)
                                    }
                                    continue MESSAGE_LOOP
                              }
                        case .notification:
                              if error.code == .methodNotFound {
                                    log(
                                          "ignoring unknown notification \(error)"
                                    )
                                    continue MESSAGE_LOOP
                              }
                        case .unknown: break
                        ...
                        fatalError(
                              "fatal error encountered decoding message \(error)"
                        )
      ...
      func handle(_ message: Message) {
            ...
            case .response(let response, let id):
                  guard
                        let outstanding = outstandingRequests.removeValue(
                        forKey: id)
                  else {
                        log("Unknown request for \(id)", level: .error)
                        return
      ...
      func send(rawData dispatchData: DispatchData) {
            ...
            sendIO.write(offset: 0, data: dispatchData, queue: sendQueue) {
                  [weak self] done, _, errorCode in
                  if errorCode != 0 {
                        log(
                              "IO error sending message \(errorCode)",
                              level: .error)
                        if done { self?.close() }
                  }
            }
            ...
            messageData.withUnsafeBytes { pointer in
                  dispatchData.append(
                        UnsafeRawBufferPointer(
                              start: pointer, count: messageData.count))
            }
      ...
      func send(encoding: (JSONEncoder) throws -> Data) {
            ...
            do {
                  ...
                  fatalError(
                        "unexpected error while encoding response: \(error)")
            }
...
extension JSONRPCConection: _IndirectConnection {
      ...
      public func send<Request>(
            _ request: Request, queue: DispatchQueue,
            ...
            let id: RequestID = self.queue.sync {
                  ...
                  outstandingRequests[id] = OutstandingRequest(
                        ...
                        responseType: Request.Response.self, queue: queue,
                        replyHandler: { anyResult in
                              queue.async {
                                    reply(
                                          anyResult.map {
                                                $0 as! Request.Response
                                          })
                              }
                        })
                  return id
      ...
      public func sendReply<Response>(
            _ response: LSPResult<Response>, id: RequestID
            ...
            send { encoder in
                  ...
                  case .success(let result):
                        return try encoder.encode(
                              Message.response(result, id: id))
                  case .failure(let error):
                        return try encoder.encode(
                              Message.errorResponse(error, id: id))
                  }
            }
      }
}
