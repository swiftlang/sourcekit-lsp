...
extension RandomAccessCollection where Element == UInt8 {
      ...
      func splitMessage() throws
            -> ((SubSequence, header: MessageHeader), SubSequence)? {
            ...
            return (
                  (rest.prefix(contentLength), header: header),
                  rest.dropFirst(contentLength)
            )
      }
      ...
      func parseHeader() throws -> (MessageHeader, SubSequence)? {
            ...
            while let (kv, rest) = try slice.parseHeaderField() {
                  guard let (key, value) = kv else { return (header, rest) }
                  slice = rest
      ...
      func parseHeaderField() throws
            -> ((key: SubSequence, value: SubSequence)?, SubSequence)? {
            if starts(with: MessageHeader.separator) {
                  return (nil, dropFirst(MessageHeader.separator.count))
            }
            else if first == MessageHeader.separator.first { return nil }

            guard
                  let keyEnd
                  = firstIndex(where: {
                  MessageHeader.invalidKeyBytes.contains($0)
            }) else { return nil }
            if self[keyEnd] != MessageHeader.colon {
                  throw MessageDecodingError.parseError(
                        "expected ':' in message header")
            }
            let valueStart = index(after: keyEnd)
            guard
                  let valueEnd = self[valueStart...].firstIndex(
                  of: MessageHeader.separator)
            else { return nil }

            return (
                  (key: self[..<keyEnd], value: self[valueStart..<valueEnd]),
                  self[index(valueEnd, offsetBy: 2)...]
            )
      }
}
