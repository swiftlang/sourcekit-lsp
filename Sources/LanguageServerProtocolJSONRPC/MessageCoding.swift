...
extension CodingUserInfoKey {
      public static let responseTypeCallbackKey: CodingUserInfoKey
            = CodingUserInfoKey(rawValue: "lsp.jsonrpc.responseTypeCallback")!
}
...
extension Message: Codable {
      ...
      public init(from decoder: Decoder) throws {
            ...
            let jsonrpc = try container.decodeIfPresent(
                  String.self, forKey: .jsonrpc)
            ...
            do {
                  let method = try container.decodeIfPresent(
                        String.self, forKey: .method)
                  let error = try container.decodeIfPresent(
                        ResponseError.self, forKey: .error)
                  ...
                  case (nil, let method?, _, nil):
                        ...
                        guard
                              let messageType
                              = MessageRegistry.shared.notificationType(
                              for: method)
                        else {
                              throw MessageDecodingError.methodNotFound(method)
                        }

                        let params = try messageType.init(
                  ...
                  case (let id?, let method?, _, nil):
                        ...
                        guard
                              let messageType
                              = MessageRegistry.shared.requestType(for: method)
                        else {
                              throw MessageDecodingError.methodNotFound(method)
                        }
                  ...
                  case (let id?, nil, true, nil):
                        ...
                        guard
                              let responseTypeCallback = decoder.userInfo[
                              .responseTypeCallbackKey] as? ResponseTypeCallback
                        else {
                              fatalError(
                                    "missing or invalid responseTypeCallbackKey on decoder"
                              )
                        }

                        guard let responseType = responseTypeCallback(id) else {
                              throw MessageDecodingError.invalidParams(
                                    "response to unknown request \(id)")
                        }

                        let result = try responseType.init(
                              from: container.superDecoder(forKey: .result))
                  ...
                  let path = context.codingPath.map { $0.stringValue }.joined(
                        separator: ".")
                  throw MessageDecodingError.invalidParams(
                        "type mistmatch at \(path) : \(context.debugDescription)",
                        id: id, messageKind: msgKind)

            }
            catch {
                  throw MessageDecodingError.parseError(
                        error.localizedDescription, id: id, messageKind: msgKind
                  )
            }
      }
