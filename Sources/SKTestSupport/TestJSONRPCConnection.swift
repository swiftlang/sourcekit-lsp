...
public final class TestClient: LanguageServerEndpoint {
      ...
      public var replyQueue: DispatchQueue = DispatchQueue(
            label: "testclient-reply-queue")
      var oneShotNotificationHandlers: [((Any) -> Void)] = []

      public var allowUnexpectedNotification: Bool = false

      public func appendOneShotNotificationHandler<N>(
            _ handler: @escaping (Notification<N>) -> Void
      ) {
            oneShotNotificationHandlers.append({ anyNote in
                  guard let note = anyNote as? Notification<N> else {
                        fatalError(
                              "received notification of the wrong type \(anyNote); expected \(N.self)"
                        )
                  }
                  handler(note)
            })
      ...
      public func handleNextNotification<N>(
            _ handler: @escaping (Notification<N>) -> Void
      ) {
      ...
      where R: RequestType { fatalError() }
}
...
extension TestClient: Connection {
      ...
      where Notification: NotificationType { server.send(notification) }
...
public func registerTestRequests() {
      [EchoRequest.self, EchoError.self,].forEach {
            MessageRegistry.shared._register($0)
      }

      [EchoNotification.self,].forEach { MessageRegistry.shared._register($0) }
}
