...
public func logAsync(
      ...
      messageProducer: @escaping (_ currentLevel: LogLevel) -> String?
) {
...
public func orLog<R>(
      _ prefix: String = "", level: LogLevel = .default,
      _ block: () throws -> R?
) -> R? {
      do { return try block() }
      catch {
...
public final class Logger {
      ...
      let logQueue: DispatchQueue = DispatchQueue(
            label: "log-queue", qos: .utility)

      /// - note: This is separate from the logging queue to make it as fast as possible. Ideally we'd use a relaxed atomic.
      let logLevelQueue: DispatchQueue = DispatchQueue(
            label: "log-level-queue", qos: .userInitiated)
      ...
      public func addLogHandler(_ handler: LogHandler) {
            logQueue.async { self.handlers.append(handler) }
      }

      @discardableResult public func addLogHandler(
            _ handler: @escaping (String, LogLevel) -> Void
      ) -> AnyLogHandler {
      ...
      public func log(_ message: String, level: LogLevel, async: Bool = true) {
            ...
            let logImpl = {
                  self.logImpl(message, level: level, usedOSLog: usedOSLog)
            }

            if async { logQueue.async { logImpl() } }
            else { logImpl() }
      }
...
#if canImport(os)
      extension LogLevel {
            @available(OSX 10.12, *) public var osLogType: OSLogType {
                  switch self {
