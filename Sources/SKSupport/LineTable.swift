...
public struct LineTable: Hashable {
      @usableFromInline var impl: [String.Index]
      ...
      /// The number of lines.
      @inlinable public var count: Int { return impl.count }

      /// Returns the given (zero-based) line as a Substring, including the newline.
      ///
      /// - parameter line: Line number (zero-based).
      @inlinable public subscript(line: Int) -> Substring {
            return content[
                  impl[line]..<(
                        line == count - 1 ? content.endIndex : impl[line + 1]
                  )]
      }

      /// Translate String.Index to logical line/utf16 pair.
      @usableFromInline func lineAndUTF16ColumnOf(
            _ index: String.Index, fromLine: Int = 0
            ...
            while true {
                  ...
                  if mid == lower || lineStartIndex == index {
                        return (
                              line: mid,
                              utf16Column: content.utf16.distance(
                                    from: lineStartIndex, to: index)
                        )
                  }
                  else if lineStartIndex < index { lower = mid }
                  else { upper = mid }
            }
      }
}
...
extension LineTable: RandomAccessCollection {
      public var startIndex: Int { return impl.startIndex }

      public var endIndex: Int { return impl.endIndex }
}
...
extension LineTable {
      ...
      @inlinable mutating public func replace(
            fromLine: Int, utf16Offset fromOff: Int, toLine: Int,
            utf16Offset toOff: Int, with replacement: String
      ) {
      ...
      mutating public func replace(
            fromLine: Int, utf16Offset fromOff: Int, utf16Length: Int,
            ...
            self.replace(
                  fromLine: fromLine, utf16Offset: fromOff, toLine: toLine,
                  utf16Offset: toOff, with: replacement)
      }
}
...
extension LineTable {
      ...
      @inlinable public func stringIndexOf(line: Int, utf16Column: Int)
            -> String.Index?
      {
            ...
            return content.utf16.index(
                  lineSlice.startIndex, offsetBy: utf16Column,
      ...
      /// - parameter line: Line number (zero-based).
      /// - parameter utf16Column: UTF-16 column offset (zero-based).
      @inlinable public func utf8OffsetOf(line: Int, utf16Column: Int) -> Int? {
            guard
                  let stringIndex = stringIndexOf(
                  line: line, utf16Column: utf16Column)
            else { return nil }
            return content.utf8.distance(
                  from: content.startIndex, to: stringIndex)
      }

      /// Returns logical position of given source offset.
      ///
      /// - parameter utf8Offset: UTF-8 buffer offset (zero-based).
      @inlinable public func lineAndUTF16ColumnOf(utf8Offset: Int)
            -> (line: Int, utf16Column: Int)? {
            guard utf8Offset <= content.utf8.count else {
                  // Offset ouf of range.
                  return nil
            }
            return lineAndUTF16ColumnOf(
                  content.utf8.index(content.startIndex, offsetBy: utf8Offset))
      }
      ...
      @inlinable public func utf16ColumnAt(line: Int, utf8Column: Int) -> Int? {
            guard line < count else {
                  // Line out of range.
                  return nil
            }
            ...
            guard
                  ...
                  lineSlice.startIndex, offsetBy: utf8Column,
                  limitedBy: lineSlice.endIndex)
            else {
                  // Column out of range
                  return nil
            }
            return content.utf16.distance(
                  from: lineSlice.startIndex, to: targetIndex)
      }
}
