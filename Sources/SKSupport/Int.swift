...
extension UInt8 {
      @inlinable public var isSpace: Bool { return isspace(Int32(self)) != 0 }

      @inlinable public var isDigit: Bool { return isdigit(Int32(self)) != 0 }

      @inlinable public var asciiDigit: Int {
...
extension Int {
      ...
      @inlinable public init?<C>(ascii buffer: C)
      where C: Collection, C.Element == UInt8 {
            ...
            // Check sign if any.
            var sign = 1
            if buffer[i] == UInt8(ascii: "+") { i = buffer.index(after: i) }
      ...
      /// - overflow will trap instead of being undefined
      /// - does not allow non-whitespace characters at the end
      @inlinable public init?<S>(ascii buffer: S) where S: StringProtocol {
