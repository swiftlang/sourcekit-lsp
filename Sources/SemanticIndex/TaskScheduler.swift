...
package actor TaskScheduler<TaskDescription: TaskDescriptionProtocol> {
  ...
  private var maxConcurrentTasksByPriority: [(priority: TaskPriority, maxConcurrentTasks: Int)] {
    didSet {
      ...
      var prioritiesOfTasksToKeep: [TaskPriority] = []
      ...
      for task in currentlyExecutingTasks.sorted(by: { $0.priority > $1.priority }) {
        ...
        if Self.canScheduleTask(
          ...
          prioritiesOfCurrentlyExecutingTasks: prioritiesOfTasksToKeep
        ) {
          prioritiesOfTasksToKeep.append(taskPriority)
        } else {
  ...
  package static func canScheduleTask(
    ...
    maxConcurrentTasksByPriority: [(priority: TaskPriority, maxConcurrentTasks: Int)],
    prioritiesOfCurrentlyExecutingTasks: [TaskPriority]
  ) -> Bool {
    if prioritiesOfCurrentlyExecutingTasks.count >= maxConcurrentTasksByPriority.first!.maxConcurrentTasks {
      return false
    }
    for (priority, maxConcurrentTasks) in maxConcurrentTasksByPriority {
      ...
      if prioritiesOfCurrentlyExecutingTasks.count(where: { $0 <= priority }) >= maxConcurrentTasks {
        return false
      }
    }
    return true
    ...
    for task in pendingTasks {
      guard
        Self.canScheduleTask(
          ...
          maxConcurrentTasksByPriority: maxConcurrentTasksByPriority,
          prioritiesOfCurrentlyExecutingTasks: currentlyExecutingTasks.map(\.priority)
        )
      else {
